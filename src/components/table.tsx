...
export const Table: FC<TableProps> = ({list, total, onChangePage}) => {
    ...
    const columns = [
        ...
        columnHelper.accessor('cover', {
            id: 'cover',
            header: '头像',
            cell: ({row}) => <img src={row.original.cover} alt={row.original.title} className="" width="30"
                                  height="30"/>
        }),
        ...
        columnHelper.accessor('is_delete', {
            ...
            cell: ({row}) => {
                return <Tag
                    color={row.original.is_delete ? 'red' : 'green'}>{row.original.is_delete ? '删除' : '正常'}</Tag>
            }
        }),
        columnHelper.accessor('is_hide', {
            ...
            cell: ({row}) => {
                return <Tag
                    color={row.original.is_recommend ? 'purple' : 'green'}>{row.original.is_hide ? '隐藏' : '显示'}</Tag>
            }
        }),
        ...
        columnHelper.accessor('is_top', {
            ...
            cell: ({row}) => {
                return <Tag
                    color={row.original.is_recommend ? 'purple' : 'green'}>{row.original.is_top ? '置顶' : '不置顶'}</Tag>
            }
        }),
        columnHelper.accessor('is_recommend', {
            ...
            cell: ({row}) => {
                return <Tag
                    color={row.original.is_recommend ? 'purple' : 'green'}>{row.original.is_recommend ? '推荐' : '不推荐'}</Tag>
            }
        }),
        columnHelper.group({
            header: '操作',
                    ...
                    <div className="flex justify-center items-center space-x-2 gap-1">
                        ...
                        <ActionButton
                            ...
                            onClick={() => updateArticle({
                                id: row.original.id,
                                is_recommend: !row.original.is_recommend
                            })}
                        />
                    </div>
                )
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id} className="bg-gray-200">
                        {headerGroup.headers.map((header) => (
                            <th key={header.id}
                                className="border border-gray-300 text-center py-3 px-4 font-semibold text-gray-700">
